...
import type { IPhonecode } from '../src/types.ts';
...
try {
  generate(sourceArg('tsx scripts/generate-data.ts <source-file-path> [output-file]'), process.argv[3] ?? join(here, '..', 'src', 'data', 'phonecodes.json'));
} catch (err) {
  fail(err);
}
